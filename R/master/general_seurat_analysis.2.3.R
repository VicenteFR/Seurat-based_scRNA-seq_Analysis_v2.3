...
option.list <- list(
  ### ------------------> Generalities.
  make_option(opt_str="--ReportsPath", type="character", default=NULL, dest="reports.path", help="Char vector, absolute path to directory to save analyses reports"),
  make_option(opt_str="--PrjName", type="character", default=NULL, dest="prj.name", help="Char vector, name to be given to the project."),
  make_option(opt_str="--DataFile", type="character", default=NULL, dest="data.file", help="Char vector, absolute path to file storing a feature by counts matrix (format provided by 10x's cellranger)."),
  make_option(opt_str="--InputType", type="character", default="matrix", dest="input.type", help="Char vector, indicates what kind of input should be considered, either 'counts' for a count matrix or 'seurat' for a seurat object."),
  make_option(opt_str="--FeatureID", type="character", default="name", dest="feature.id", help="Char vector, defines the feature ID type to take from the counts matrix. Default is 'name' (features.tsv column 2). Possible values are 'name' and 'ensembl', the last one corresponding to the Ensembl ID. This option applies only if a count matrix (and not a seurat object) is provided.\n"),
  make_option(opt_str="--Raw10x", type="character", default=NULL, dest="tenx.raw.data", help="Char vector, absolute path to 10x raw data path (as a directory instead of an hf5 matrix file). Necessary when \"ensembl\" is picked as the feature ID option.\n"),
  make_option(opt_str="--IsFivePrime", type="logical", dest="is.five.prime", default=FALSE, help="Logical, indicates whether the data comes from a five-prime experiment, in which case, antigen receptor (either T-cell receptor or B-cell receptor) genes will be excluded from downstream analyses."),
  ...
  make_option(opt_str="--MarkersFile", type="character", dest="markers.file", default=NULL, help="Char vector, absolute path to RDS file listing gene markers. These markers will be assessed. Format is described elsewhere and an example provided along.\n"),
  make_option(opt_str="--RAM", type="integer", dest="avl.ram", default=NULL, help="Int, indicates available RAM for the job session."),

  ### ------------------> For Annotations.
  make_option(opt_str="--DoAnnotate", type="logical", default=FALSE, dest="do.annotate", help="Logical, indicates whether to perform metadata annotation based on aggr table (format detailed elsewhere).\n."),
  make_option(opt_str="--AggrTable", type="character", default=NULL, dest="aggr.table.file", help="Char vector, absolute path to fully-annotated aggregation table. Same format as a cellranger aggr table, but with at least one of next tags: hto.tag, lane.tag,  chromium.batch.tag or seq.batch.tag as annotations for each sample. Then, each row describes the info for each 10x lane (lane.tag) per pairs of chromium platform batch and sequencing batch (batch.tag), and hto.tag column lists the absolute paths to the ClassificationPerCell.csv file output by the HTO deconvolution (or similar) pipeline."),
  make_option(opt_str="--DonorsMetaData", type="character", default=NULL, dest="donors.data.file", help="Char vector, absolute path to donors metadata file. Set as NULL if there's none.\n"),
  ...
  make_option(opt_str="--MergeBy", type="character", default=NULL, dest="donors.merge.by", help="Char vector, column name in both seurat object meta data and donors meta data in order to merge them and add further annotations. Value considered only if there are annotations and donors metadata provided.\n"),
  make_option(opt_str="--LaneID", type="character", default='lane.id.tag', dest="lane.tag.id", help="Char vector, represents teh names to be given to the columns listing the values defined in the column 'lane.tag.id' of the aggregation table. Must be a single string where different column names are separated by semicolons.\n"),
  make_option(opt_str="--MultAnnPerLane", type="logical", default=FALSE, dest="mult.ann.per.lane", help="Logical, indicates wether the lane tag column in the aggregation table lists multiple values. If TRUE, they should be separated by semicolon within each aggregation table lane index and there should be the same amount of values per library (i.e., per row in the aggregation table)."),
  ...
  ### ------------------> Subsetting options.
  make_option(opt_str="--DoPreSubset", type="logical", default=FALSE, dest="do.presubset", help="Logical, indicates whether presubsetting should be applied or not. Only relevant when a seurat object is input. Otherwise, the value is completely ignored.\n"),
  make_option(opt_str="--PreSubsetCriteria", type="character", default=NULL, dest="presubset.criteria.file", help="Char vector, absolute path to file indicating the criteria used to apply presubsetting. Only relevant when a seurat object is input. Two different subset strategies are valid depending on the columns defined in the file. If the file has a single column (named \"barcode\"), then the barcodes listed there will be taken as the subset to be kept for downstream analyses. Otherwise, the file has to have the same format as for Tag and Features criteria (see options 'DoSubset', 'TagsCriteria' and 'FeatsCriteria' below) and the same subsetting strategy is applied.\n."),
  make_option(opt_str="--DoSubset", type="logical", default=FALSE, dest="do.subset", help="Logical, flag indicates whether the seurat object must be filtered by excluding a subset of cells or a subset of genes/features. Rules to remove or keep cells are specified through options 'TagsCriteria' and 'FeatsCriteria' (see below)."),
  make_option(opt_str="--TagsCriteria", type="character", default=NULL, dest="tags.criteria.file", help="Char vector, absolute path to file indicating the criteria used to apply presubsetting according to barcodes. Two different subset strategies are valid depending on the columns defined in the file. If the file has a single column (named \"barcode\"), then the barcodes listed there will be taken as the subset to be kept for downstream analyses. Otherwise, the file has to have the same format as for Tag and Features criteria and the same subsetting strategy is applied\n."),
  make_option(opt_str="--FeatsCriteria", type="character", default=NULL, dest="feats.criteria.file", help="Char vector, absolute path to table storing the criteria to filter cells based on for tags required (format: criteria by tag, column name should match that of tag defined in seurat object; criteria should be separated in two different rows, lower and upper threshold respectively)."),

  ### ------------------> For QCs.
  make_option(opt_str="--minCounts", type="integer", default=300, dest="min.nCounts", help="Integer, indicates the lower threshold for number of UMIs per cell.\n"),
  make_option(opt_str="--maxCounts", type="integer", dest="max.nCounts", help="Integer, indicates the upper threshold for number of UMIs per cell.\n"),
  make_option(opt_str="--minFeatures", type="integer", default=200, dest="min.nFeatures", help="Integer, indicates the lower threshold for number of genes per cell.\n"),
  make_option(opt_str="--maxFeatures", type="integer", default=2500, dest="max.nFeatures", help="Integer, indicates the upper threshold for number of UMIs per cell.\n"),
  make_option(opt_str="--maxMP", type="numeric", default=0.05, dest="max.MP", help="Integer, indicates the upper threshold for percentage of mitochondrial UMIs per cell.\n"),
  make_option(opt_str="--FilterOut", type="logical", dest="filter.out", default=TRUE, help="Logical, indicates whether the barcodes identified to have low quality (based on the criteria defined with the options above) must be filtered out for downstream analysis.\nFor a pilot run, it is highly recommend to keep even the low-quality cells.\n"),

  ### ------------------> For normalization.
  make_option(opt_str="--FVFsMethod", type="character", default="vst", dest="FVFs.method", help="Char vector, indicates the method to use to find variable features"),
  make_option(opt_str="--FeatsForDSA", type="integer", default=30, dest="feats.for.dsa", help="Int, indicates one of two options depending on whether the value is over 100. If larger than 100, it indicates the amount of variable features to take into consideration for downstream analysis. If smaller or equal to 100, it indicates the percentage of standardized variance to be considered such that the most variable features that account for that percentage of variance will be taken into consideration.\n"),
  make_option(opt_str="--MeanCutoff", type="numeric", dest="mean.cutoff", default=0.1, help="Mean cutoff to exclude variable features."),
  ...
  make_option(opt_str="--GenNormMethod", type="character", default="LogTransform", dest="gen.norm.method", help="Char vector, indicates the method to use for general data normalization. Possible values (other than these, program will stop): LogTransform (Log-normalization for cell-wise normalization proceeded by other steps such as gene normalization) and SCTransform (normalization and variance stabilization through Regularized Negative Bonimial Regression per gene and use of pearson residuals, see Hafemeister, C., & Satija, R. (2019). Normalization and variance stabilization of single-cell RNA-seq data using regularized negative binomial regression. Genome Biology, 20(1), 1-15.)."),
  make_option(opt_str="--NormMethod", type="character", default="LogNormalize", dest="norm.method", help="Char vector, indicates the method to use for data normalization per cell. If SCTransform is selected as the general normalization method, this will be ignored."),
  ...
  make_option(opt_str="--ForHarmony", type="character", default=NULL, dest="do.harmony", help="Char vector, if different than NULL, it indicates that Harmony batch-effect removal should be applied. Then, this may be evaluated as a character vector of the metadata column listing the batches to be regressed out through Harmony.\n"),
  make_option(opt_str="--Resolution", type="character", default="c(0.2, 0.4, 0.6)", dest="resolutions", help="Char vector. A single string to be interpreted as a numeric vector where each value must be a nonnegative value between 0.1 and 1. For example, the string 'c(0.2, 0.4)' will be interpreted as a numeric vector with entries 0.2 and 0.4. These indicate the resolutions to consider during clustering. There is not a limit in the number of resolutions to try, but be aware that the more you provide, the longer it will take for the script to run.\n"),
  make_option(opt_str="--DimReduction", type="character", dest="dim.reduction.methods", default="c('umap', 'tsne')", help="String to be evaluated as a character vector indicating the dimensionality reduction methods that should be applied over the data other than PCA (which will run tried all the same). Possible values are 'umap' and 'tsne', where either or both can be provided. Default is both.\n"),
  make_option(opt_str="--MaxCellsNo", type="integer", dest="max.cells.no", default=5000, help="Int, indicates the maximum number of cells to depict in plots."),

  ### ------------------> For Differential expression analysis.
  make_option(opt_str="--DEA", type="logical", dest="do.dea", default=FALSE, help="Logical indicating if pairwise comparisons (DEA) should be applied and reported or not."),
